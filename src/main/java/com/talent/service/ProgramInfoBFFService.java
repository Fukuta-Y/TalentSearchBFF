...
public class ProgramInfoBFFService
{
    ...
    public ProgramInfo getProgramInfo(String programId)
    {
        ...
        for (MKbnGenre e : mKbnGenreList)
        {
            ...
            if (
                StringUtils.equals(e.getGenreId().toString(), "1")
                    && e.getJyunjyo().compareTo(prmList.getmProgram().get(0).getGenreId()) == 0
            )
            {
                // ジャンル名
            ...
            if (
                StringUtils.equals(e.getGenreId().toString(), "3")
                    && e.getJyunjyo().compareTo(prmList.getmProgram().get(0).getChannelId()) == 0
            )
            {
                // チャンネル名
